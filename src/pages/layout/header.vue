...
<template>
	...
	<header class="main-header animated" :class="{closeLogo:sidebar.collapsed,mobileLogo:device.isMobile}">
		<!-- Logo -->
		<a href="#" class="logo">
			<span class="logo-lg">&nbsp;&nbsp; <b style="margin-left: 14px;">urmp</b></span>
		</a>
						...
						<ul class="dropdown-menu" v-if="showProfileBox" @mouseleave="test">
							...
							<li class="user-footer">
								<div class="pull-left">
									...
									<el-button @click="add" type="default">修改密码</el-button>
									<!--</router-link>-->
								</div>
		...
		<!--修改密码-->
		<el-dialog custom-class="dialog_content" title="修改密码" :modal="false" :visible.sync="dialogFormVisibles" size="small">
		...
		methods: {
			save() { //修改密码
				...
				if(newPwd == "" || oldPwd == "") {
					...
					this.$http.get(auth.getServerUrl() + api.CHANGE_PWD + "?userpwd=" + newPwd + "&orderPwd=" + oldPwd)
						.then(res => {
